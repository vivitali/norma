...
describe("amortization", () => {
  ...
  it("re-amortizes over the REMAINING years at renewal, not the original term", () => {
    ...
    const a = amortization(ca, { ...base, renewalRate: 7 });
  ...
  it("lowers the payment when renewal lands on a lower rate", () => {
    const a = amortization(ca, { ...base, renewalRate: 2 });
  ...
  it("renews once per term, at the start of each new one", () => {
    const a = amortization(ca, { ...base, renewalRate: 5, termYears: 5 });
...
describe("marginalRate", () => {
  ...
  it("returns the top rate above the last cap", () => {
    expect(marginalRate(ca, "ON", 5_000_000)).toBe(0.5353);
  });
});
...
describe("waterfall", () => {
  ...
  it("draws in the fixed cost order: free, then strings, then taxed", () => {
    const w = waterfall(ca, { ...base, need: 30000, fhsa: 10000, cash: 10000, rrsp: 60000, tfsa: 10000 });
  ...
  it("spends a gift before an HBP withdrawal or a TFSA", () => {
    ...
    const w = waterfall(ca, { ...base, need: 30000, gift: 30000, rrsp: 60000, tfsa: 30000 });
    const drawn = w.rows.filter((r) => r.drawn > 0).map((r) => r.key);
  ...
  it("draws neither FHSA nor HBP money for a buyer who is not a first-time buyer", () => {
    ...
    const w = waterfall(ca, { ...base, ftb: false, need: 100000, fhsa: 40000, rrsp: 60000 });
  ...
  it("keeps the blocked rows present, with a reason, rather than dropping them", () => {
    ...
    const w = waterfall(ca, { ...base, ftb: false, fhsa: 40000, rrsp: 60000 });
  ...
  it("marks nothing blocked for a first-time buyer", () => {
    const w = waterfall(ca, { ...base, fhsa: 40000, rrsp: 60000 });
    expect(w.rows.every((r) => r.blocked === undefined)).toBe(true);
  });

  it("caps the HBP draw at the federal maximum, not at the RRSP balance", () => {
    const w = waterfall(ca, { ...base, need: 200000, rrsp: 200000 });
    expect(w.rows.find((r) => r.key === "hbp")!.drawn).toBe(ca.hbp.max);
  });

  it("creates a 15-year repayment obligation sized to what was drawn", () => {
    const w = waterfall(ca, { ...base, need: 30000, rrsp: 60000 });
    const hbp = w.rows.find((r) => r.key === "hbp")!;
    expect(hbp.repayAnnual).toBeCloseTo(30000 / ca.hbp.repayYears, 6);
  });
  ...
  it("reports a surplus when the sources exceed the need", () => {
    const w = waterfall(ca, { ...base, need: 20000, cash: 50000 });
    expect(w.shortfall).toBe(0);
    expect(w.surplus).toBe(30000);
  });

  it("never draws from a negative balance", () => {
    const w = waterfall(ca, { ...base, need: 10000, cash: -5000, fhsa: 10000 });
...
describe("glidePath", () => {
  it("reaches the target and reports the month it happened", () => {
    const g = glidePath(ca, 12000, 1000, 36);
  ...
  it("returns null rather than a rounded-up month when the target is out of reach", () => {
    ...
    const g = glidePath(ca, 500000, 200, 36);
    expect(g.reach).toBeNull();
  });

  it("starts at zero and never goes backwards", () => {
    const g = glidePath(ca, 10000, 300, 24);
  ...
  it("compounds, so it beats plain monthly saving times the months", () => {
    const g = glidePath(ca, 0, 1000, 36);
...
describe("taxOnBand", () => {
  ...
  it("agrees with the marginal rate on an infinitesimal band", () => {
    const at = 90000;
    const band = taxOnBand(ca, "ON", at, at + 1);
    expect(band).toBeCloseTo(marginalRate(ca, "ON", at + 1), 9);
  });

  it("is additive across a split, so no bracket boundary is double-counted or skipped", () => {
    const whole = taxOnBand(ca, "BC", 0, 250000);
    const parts =
      taxOnBand(ca, "BC", 0, 98560) +
      taxOnBand(ca, "BC", 98560, 181400) +
      taxOnBand(ca, "BC", 181400, 250000);
    expect(parts).toBeCloseTo(whole, 6);
  });

  it("returns nothing for an empty or inverted band, and clamps a negative floor to zero", () => {
    expect(taxOnBand(ca, "ON", 50000, 50000)).toBe(0);
    expect(taxOnBand(ca, "ON", 75000, 15000)).toBeCloseTo(
      taxOnBand(ca, "ON", 15000, 75000),
      9,
    );
    expect(taxOnBand(ca, "ON", -20000, 10000)).toBeCloseTo(
      taxOnBand(ca, "ON", 0, 10000),
      9,
    );
  });
  ...
  it("keeps running above the final bracket, whose ceiling is null", () => {
    const top = ca.marginal.ON[ca.marginal.ON.length - 1][1];
    expect(taxOnBand(ca, "ON", 1_000_000, 1_100_000)).toBeCloseTo(100000 * top, 4);
  });
});
...
describe("hbpPlay", () => {
  ...
  it("caps the contribution at the federal maximum", () => {
    const h = hbpPlay(ca, { ...ON, contribution: 100000, withdrawAmount: 100000 });
    expect(h.contribution).toBe(ca.hbp.max);
    expect(h.withdraw).toBe(ca.hbp.max);
  });

  it("cannot withdraw more than was contributed", () => {
    const h = hbpPlay(ca, { ...ON, contribution: 20000, withdrawAmount: 60000 });
  ...
  it("still prices the FIRST dollar of the deduction at the marginal rate", () => {
    const h = hbpPlay(ca, { ...ON, contribution: 1, withdrawAmount: 1 });
    expect(h.refund).toBeCloseTo(marginalRate(ca, "ON", 75000), 6);
  });
  ...
  it("never refunds tax on income that was never earned", () => {
    ...
    const h = hbpPlay(ca, { prov: "ON", income: 20000, contribution: 60000, withdrawAmount: 60000 });
    expect(h.refund).toBeCloseTo(taxOnBand(ca, "ON", 0, 20000), 6);
  });

  it("repays the whole withdrawal over the statutory years, to exactly zero", () => {
    const h = hbpPlay(ca, { ...ON, contribution: 60000, withdrawAmount: 60000 });
    expect(h.schedule).toHaveLength(ca.hbp.repayYears);
  ...
  it("prices a missed repayment year at the marginal rate on the missed amount", () => {
    ...
    const h = hbpPlay(ca, { ...ON, contribution: 60000, withdrawAmount: 60000 });
    const rate = marginalRate(ca, "ON", 75000);
    expect(h.marginalRate).toBe(rate);
    expect(h.taxIfMissed).toBeCloseTo((60000 / ca.hbp.repayYears) * rate, 6);
  });
  ...
  it("says when the withdrawal was cut back to the contribution", () => {
    ...
    const h = hbpPlay(ca, { ...ON, contribution: 0, withdrawAmount: 60000 });
  ...
  it("names the contribution even where the federal maximum ties with it", () => {
    ...
    const h = hbpPlay(ca, { ...ON, contribution: 100000, withdrawAmount: 200000 });
    expect(h.contribution).toBe(ca.hbp.max);
    expect(h.withdraw).toBe(ca.hbp.max);
    expect(h.clampedByContribution).toBe(true);
  });
  ...
  it("claims no clamp when the reader asked for less than they contributed", () => {
    const h = hbpPlay(ca, { ...ON, contribution: 60000, withdrawAmount: 20000 });
...
describe("rentVsBuy", () => {
  ...
  it("stops the mortgage payment at payoff, which is where the buy line steepens", () => {
    ...
    const r = rentVsBuy(toronto, ca, base);
  ...
  it("nets selling cost out of equity, because unrealisable wealth is not wealth", () => {
    ...
    expect(row.equity).toBeCloseTo(row.homeValue * (1 - ca.sellingCost) - row.balance, 4);
  });

  it("holds the house flat when appreciation is switched off", () => {
    const r = rentVsBuy(toronto, ca, { ...base, appreciationOn: false });
  ...
  it("credits neither side with the difference when investing it is switched off", () => {
    const r = rentVsBuy(toronto, ca, { ...base, investDiff: false });
...
describe("scenario", () => {
  ...
  it("raises a below-minimum request to the legal floor and says it did", () => {
    const s = scenario(toronto, ca, { ...base, dpPct: 3 });
    expect(s.belowMinimum).toBe(true);
    expect(s.down).toBeCloseTo(minDown(ca, base.price), 6);
    expect(s.dpPctEff).toBeGreaterThan(3);
  });

  it("leaves a legal request alone", () => {
    const s = scenario(toronto, ca, { ...base, dpPct: 20 });
    expect(s.belowMinimum).toBe(false);
    expect(s.dpPctEff).toBeCloseTo(20, 6);
  });

  it("stops insuring at 20% down", () => {
    expect(scenario(toronto, ca, { ...base, dpPct: 19 }).insured).toBe(true);
    expect(scenario(toronto, ca, { ...base, dpPct: 20 }).insured).toBe(false);
    expect(scenario(toronto, ca, { ...base, dpPct: 20 }).premium).toBe(0);
  });
  ...
  it("recomputes closing costs per column where the province taxes the premium", () => {
    ...
    // being compared.
    const low = scenario(montreal, ca, { ...base, dpPct: 5 });
    const high = scenario(montreal, ca, { ...base, dpPct: 20 });
  ...
  it("charges no premium tax where the province does not levy one", () => {
    expect(scenario(calgary, ca, { ...base, dpPct: 5 }).premiumTaxLine).toBe(0);
  });

  it("returns null, not zero, for fundability when funds were never given", () => {
    const s = scenario(toronto, ca, { ...base, funds: null });
  ...
  it("returns null months when there is a shortfall and no saving rate", () => {
    const s = scenario(toronto, ca, { ...base, funds: 1000, save: null });
    expect(s.fundable).toBe(false);
    expect(s.months).toBeNull();
  });

  it("costs more to borrow at a lower down payment, premium included", () => {
    const low = scenario(toronto, ca, { ...base, dpPct: 5 });
    const high = scenario(toronto, ca, { ...base, dpPct: 20 });
  ...
  it("qualifies against the stress rate, never the contract rate", () => {
    const s = scenario(toronto, ca, base);
  ...
  it("agrees with financing() about the premium at the same effective percentage", () => {
    const s = scenario(toronto, ca, { ...base, dpPct: 10 });
    const f = financing(ca, { price: base.price, dpPct: 10, amortYears: 25 });
...
describe("rentVsBuy — the renter invests what the buyer actually produces", () => {
  ...
  const base = (ftb: boolean, dpPct: number) => ({
    ...
    rentInflation: 0.03,
    years: 10,
    appreciation: ca.appreciation.shelter,
    appreciationOn: true,
    investReturn: ca.investReturn.balanced,
  ...
  it("tracks the down-payment slider, less the closing costs a bigger down payment avoids", () => {
    const low = rentVsBuy(toronto, ca, base(true, 10));
    const high = rentVsBuy(toronto, ca, base(true, 20));
  ...
  it("invests the closing costs too — the buyer needs that cash upfront as well", () => {
    const r = rentVsBuy(toronto, ca, base(true, 10));
  ...
  it("agrees to the dollar with the Closing Costs page's own answer", () => {
    ...
    expect(rentVsBuy(toronto, ca, o).upFront).toBe(closingTotal(toronto, ca, o).net);
  });

  it("leaves a non-first-time buyer's upfront cash unchanged", () => {
    const r = rentVsBuy(toronto, ca, base(false, 10));
