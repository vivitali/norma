...
describe("AppHeader", () => {
  ...
  it("keeps the settings in the header rather than inside the menu, at every width", async () => {
    // Issue #9's constraint, stated as behaviour: the jurisdiction picker, country switcher,
    // locale switcher and theme toggle are settings, not destinations, so they live outside
    // the nav panel and stay present whether it is open or closed. The single-row/two-row
    // split that makes that fit at 320px is CSS, which jsdom cannot measure -- this pins the
